...
(function () {
  ...
  roll.prototype.parse = function (s) {
	...
	for (var seg = 1; seg < segments.length; seg++) {
	    ...
	    if (transformationParameter.indexOf('d') > -1) {
		  transforms.push(transformationKeys[operator](parseInt(new roll().roll(transformationParameter).result, 10)));
	    } else {
	...
	return {
		...
		transformations: hasTransformation || transforms.length ? transforms.length > 1 ? transforms : transformationKeys[match[4]](parseInt(match[5], 10)) : ['sum'],
		toString: function () {
		        return s;
		    }
		};
  ...
  roll.prototype.roll = function (input) {
    if (!input) {
      ...
      var resultset = [];
      for (var i in input){
       resultset.push(new roll().roll(input[i]));
      }
      return resultset;
    }
