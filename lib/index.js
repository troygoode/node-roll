    ...
    var match = regex.exec(s),
      ...
      segments = s.split(/[\+\-\/\*]/);
		      ...
		      return {
		          ...
		          transformations: hasTransformation ? transforms.length > 1 ? transforms : transformationKeys[match[4]](parseInt(match[5], 10)) : ['sum'],
		          toString: function () {
		              return s;
		          }
		      };
        ...
        transformationAdditionalParameter;
   		          ...
   		          if (typeof transformation === 'function') { // lets you pass something custom in
		              ...
		              if (transformation[0] instanceof Array) {
		                  transformationFunction = transformationFunctions[transformation[0][0]]; // fn for "add"
		                  transformationAdditionalParameter = transformation[0][1]; // 3
		              } else if (transformation[1] instanceof Array) {
		                  transformationFunction = transformationFunctions[transformation[1][0]]; // fn for "add"
		                  transformationAdditionalParameter = transformation[1][1]; // 3                     
		              } else {
